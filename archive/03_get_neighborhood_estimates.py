...
block_group_estimates = pd.read_csv("processed_data/block_group_estimates.csv").drop("geometry", axis = "columns")
...
fig = px.choropleth(result2, geojson = result2.geometry, locations = result2.index,  # Use the DataFrame index as locations
                    color=result2['weighted_median_income'],  # Replace with the actual column name for neighborhood names
